...
const compileServerSide = () => {
  ...
  const compiler = new TypescriptCompiler(process.cwd(), 'server/tsconfig.json', TS);
...
export const handler = async () => {
  ...
  try {
    ...
    const env = Object.fromEntries(Object.entries(envs).filter(([name, value]) => name.startsWith('PUBLIC_')));

    const config = createConfiguration({
      ...DefaultSnowpackConfig,
      env,
      plugins: [['@snowpack/plugin-postcss', { config: join(process.cwd(), 'config', 'postcss.config.js') }]],
    });
